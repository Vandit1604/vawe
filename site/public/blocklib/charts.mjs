...
// copy of it). `needle`/`ticks`/`glow` are additions either mode can carry.
export function gauge({ x, y, w = 300, h, value = 0, max = 100, label = '', color = TOKENS.accent,
  start = 0, dur = 4, bare = false, needle = false, ticks = 0, glow = false } = {}) {
  ...
  // over the layer's window (0 → pct), and the dash length (and, with `needle`, the pointer's own
  // rotation) is computed from it in CSS, so the motion is the thing the block is FOR, which is what
  // makes the registry browsable: you see what a gauge does, not that a card can rise. Deterministic:
  // --p is a pure function of t.
  ...
  const html = htmlCard({ w, pad: CHART_PAD, align: 'center',
    body: (inner) => gaugeSvg({ w: inner, pct, color, needle, ticks, glow })
    + `<div style="${numCss({ size: TYPE.head })};margin-top:-2px">${reading}</div>`
