#!/usr/bin/env node
// .claude/hooks/no-blanket-git.mjs — refuse the four git commands that destroyed work in this repo.
//
  ...
  for (const [re, name, why] of BANNED) {
    if (parts.some((p) => re.test(p))) {
      process.stderr.write(`BLOCKED: ${name}\n\n${why}\n\n`
        + 'Refused by .claude/hooks/no-blanket-git.mjs, because this command has already caused real\n'
