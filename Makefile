...
# make demo Q="what this shows" [NAME=<slug>] [FX=<arsenal key>]: scaffold a SPECIMEN scene and run
# the dev loop on it. A demo written from a blank file comes out a contact sheet every time (27 of the
# 35 `_*.json` scratch scenes are one), so the archetype arrives with the file: one subject full bleed,
# two backdrops, one cut, one camera move, a hand-keyed track. Nine variants of one effect is
# `make catalog`. The scaffold prints the path it wrote on stdout and its notes on stderr.
demo: ## scaffold a SPECIMEN demo scene (one subject, one shot) and iterate on it
	@f=$$(node scripts/dev/demo.mjs --print-path --q "$(Q)" $(if $(NAME),--name "$(NAME)") $(if $(FX),--fx "$(FX)")) \
