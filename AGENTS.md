...
## THE EIGHT STAGES, IN ORDER  `[ref: make stage]`

**Read this first.** `make stage D=<film>` names the stage a film is in now, and the ONE next
command. It reads the files on disk, never a stored state, so it cannot disagree with the repo. Run
one step at a time with `make next D=<film>`.
...
| 7 | **render** | build the mp4 and check every canvas | `make ship D=` |
| 8 | **judge** | the only step that SEES | `make judge D=` → `make ledger D=` |

Everything below is reference. Each section names the stage that sends you there.

**No templates.** You do not pour data into a canned layout; you compose each video from the
vocabulary in `docs/PRIMITIVES.md` (stages 2, 4). Your job when asked to "make a video about X" is to
**write a scene JSON** (and capture the real assets it needs), then render it. You do **not** edit
`scene.html` or the Go renderer unless explicitly asked.
...
Skill tool; other agents open the doc in the last column and follow it as a checklist.
...
sets three dials (`DESIGN_VARIANCE`/`MOTION_INTENSITY`/`VISUAL_DENSITY`) from **taste-skill**; other
agents set them by hand. Prefer to **capture** a real surface over inventing one, then gate it clean:
`make designspec-check D=<file>`. Name ONE art direction ("clean modern SaaS" is banned); asymmetry
and scale contrast are defaults; type is distinctive.
...
every target grouped that way, so it cannot drift from the real target list. Every target is
`.PHONY`: this is a command catalogue, not a build graph, on purpose
([`docs/MAKEFILE-AUDIT.md`](docs/MAKEFILE-AUDIT.md)).

```bash
make list                        # every target, grouped by phase
make formats                     # the scene module + its schema/sample
...
**On demand, ONE front door:** `make arsenal Q="…"` searches every named effect/block:
`AT="layers[].motion[]"` answers what is legal at one path, `SHAPE=pan|blast|drift|enter|exit` emits a
hand-keyed motion track, `BLUEPRINTS=1` catalogs directed-motion beats, `PREVIEWS=1`/`PRESETS=1` open
rendered sheets, `MISTAKES=1 Q="…"` asks the mistake log, `THEME=<name>` renders one theme's look.
`make demo Q="…"` writes a ten-second film about one thing
([`docs/CRAFT/SPECIMEN.md`](docs/CRAFT/SPECIMEN.md)). Full index:
[`docs/CRAFT/README.md`](docs/CRAFT/README.md).
...
judgements (what a beat shows, what moves, what a cut means, what the copy earns), and they do not
fit in one context at once. So a real film is made by a roster, with two kinds of member.

**A DECIDER writes into the film. A CRITIC only reports.** A critic that can fix will fix instead of
finding, and six agents writing to one file produce a film nobody chose.

**A decider owns ONE exclusive write scope and touches nothing else,** so two deciders cannot
collide and a wrong decision reverts in one field. **A role earns its place only if the engine cannot
already decide it**: type, colour, layout and backdrop resolve from the theme, so a decider there
would re-decide what the brand already decided.
...
**`make critics D=<file> DECIDERS=1` prints these six as briefs, filled in for that film.** The scene
deciders run in parallel, one per fragment; every other row reads what the row above it decided.
Motion runs before transitions because the content-aware cut reads the velocity at a joint as its
strongest signal, and a cut chosen before the motion exists is choosing against a still frame.

**Then the critics, in ONE parallel message,** each handed an artifact the author has not already
seen: beat, bg-motion, reveal, fidelity, copy, seam. `make critics D=<file>` emits them as ready
prompts with the real sheet paths filled in. A critic is EVIDENCE, never a ruling: look before you
act, and if your own eye confirms the flaw, that is a fix, never a rationalisation.

**This is not free, and it is usually overkill.** A measured fan-out here ran to 87 agents and 5.66M
tokens; one well-briefed agent beats a fan-out for anything sequential
([`docs/CRAFT/SUBAGENT-BUDGET.md`](docs/CRAFT/SUBAGENT-BUDGET.md), read it first). The roster earns
its cost on a film with real scenes: a full authoring pass, any recreation, anything you intend to
ship. Below that, do it yourself. Full roster and verdict shapes:
...
the film, ask before building. Two more lines are yours to fill, every time, unasked:
...
(`node scripts/gates/ledger.mjs not <theme>`), so it excludes the real pattern. It is a constraint,
not a ban: a beat with a real reason to repeat a value still can, waived in the scene with `_why`.
...
scripts/dev/library-stats.mjs` and cite what it prints. Content spine: hook → suspense → payoff. Never
spoil the payoff; order beats toward the most counterintuitive moment; be honest, on-screen copy must
be true, and a specific fact beats a dry number.

## Stage 4, design: rules the gates can't fully see  `[eye]`

- No CSS `animation`/`transition`/`opacity`/`filter` (the engine owns the clock); use `parts`, a
  selector giving every matched element a seeked, staggered entrance.
...
- **Show, don't only tell.** DECORATION dresses the frame; EXPLANATION does work words can't.
...
- **Real assets first, emoji last.** `make capture` for real UI, `make assets` for logos (never a bare
  `curl`, a 404 writes a zero-byte file). Never embed copyrighted material.
...
- **Launch videos:** crawl EVERY page, not the homepage. Give the logo real size (~100px+ beside a
  ...
  `slide-left`). Blur out (`out:"defocus"`) when moving would fight dense content. Put a changing word
  in a fixed-width chip so nothing after it reflows.
