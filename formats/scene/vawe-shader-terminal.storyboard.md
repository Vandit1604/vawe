...
object_last: three equal panels on one baseline, dissolving as the wordmark arrives
...
not: no voiceover, no zoom or push into the shader, no gradient hero, no centred type, no stock footage, no dots-in-a-title-bar terminal cliche
format: 1920x1080
...
- shot: medium (the panel fills the middle two thirds, the black around it deliberately empty)
- camera: a very slow pull, 1.02 to 1.0 across the first six seconds
- layout: one line of type above the panel, sharing its left edge; the panel in the middle band
- picture: a dark terminal, one white hairline at 0.10 alpha, a cobalt prompt sigil, a caret
- mechanism: a `typing` text layer at 22 characters a second, which is also what makes the key clicks
- style: true black, one mono face, one cobalt accent
...
- camera: the pull settles to 1.0
...
- picture: a banded field, cobalt to white, exactly inside the terminal's body, with the chrome and the output still over it
...
- picture: the folded terminal in the centre, a matrix rain on the left, a bokeh drift on the right
...
- picture: a mirrored mandala, a starfield, an aurora wash: three looks nobody could mistake for each other
...
- picture: a 200px wordmark with the panels dissolving behind it
