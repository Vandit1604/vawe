...
audience: "the doctrine's own before/after harness (docs/EVALS.md); nobody signs off on a fixture"
...
threads: "none: a single continuous beat has nothing to hand off across, so `docs/CRAFT/PER-SCENE-FANOUT.md`'s contract (object_in/object_out chaining across beats) does not apply here"
...
craft:
    ...
    imagery: "the mark itself is the only picture: an inline svg path, never a rasterised file (docs/RULES/svg-inline.md)"
    show-dont-tell: "n/a at this length: a sting shows the mark, it does not argue a claim"
    sound: "silent, waived: one cue could carry the mark's resolve, but this fixture stays a liveness proof, not a sound decision (docs/CRAFT/SOUND.md)"
