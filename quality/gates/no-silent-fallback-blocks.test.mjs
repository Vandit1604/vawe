...
// fallback-to-default pattern, in the blocks/blueprints component library. Lives here rather than
...
const blueprints = (f) => path.join(ROOT, 'blueprints', f);
...
const { verdictChip } = await import(`file://${blueprints('kit.mjs')}`);
...
throwsUnknown(() => verdictChip({ text: 'x', tone: 'okey' }), 'blueprints/kit.mjs verdictChip tone');
...
assert.doesNotThrow(() => verdictChip({ text: 'x', tone: 'ok' }));

console.log('ok - no-silent-fallback-blocks: 7 name-keyed sites refuse an unknown name instead of silently defaulting');
