...
// A test runner, same shape as lib-test.mjs (which the output contract exempts): each `ok()` is a
// pinned regression assertion, not a scan over one input's findings. A failure still becomes a record
// (`f.fail`), so --json gets the worklist, but the pass/fail tally stays the prose a maintainer reads.
...
ok(!bad.some((w) => /typing.*markup/i.test(w)), 'rule 2 retired: typing + <b>/<em> is silent (HTML-safe typing, #85)', 'typing-markup-retired');
ok(bad.some((w) => /colliding/.test(w)), 'rule 3: scene collision (Preferences↔agents overlap)', 'scene-collision-rule');
...
ok(disc.length === 2, `child window: both an own start/duration and a NESTED one are reported (got ${disc.length}: ${disc.join(' | ') || 'none'})`, 'child-window-discard-count');
ok(disc.some((w) => /#kid.*`start` and `duration`/.test(w)), 'child window: names the child and both props it discarded', 'child-window-names-props');
ok(disc.every((w) => /`delay`/.test(w)), 'child window: every message names the prop that DOES work', 'child-window-names-fix');
...
ok((() => { try { resolveEasing('nope-not-real'); return false; } catch (e) { return /unknown easing/.test(e.message); } })(),
  'resolveEasing: unknown name THROWS and names itself (#367 superseded #83)', 'easing-unknown-throws');
ok(resolveEasing('spring') !== easeOutCubic && typeof resolveEasing('spring') === 'function', 'resolveEasing: known name resolves to its own curve', 'easing-known-resolves');
