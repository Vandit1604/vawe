...
const WAIVED = new Map(Object.entries({
  ...
  KIND: 'which studio timeline lane each layer type sits in (core/layers/kinds.js). A lookup the studio and the site read, not a capability an author names.',
  ...
  GENERATORS: 'the ARRAY view of the ready generators. `GENERATOR_ENTRIES` is the same set as a map and IS the registry (core/generators/generators.js), so this is one vocabulary in two shapes, not two. Identity cannot see that: the array is built by a filter and is a different object. The playground reads the array because it wants the order',
