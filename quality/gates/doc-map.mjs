...
//   node quality/gates/doc-map.mjs --write    regenerate every index view (make site X=doc-index)
//
...
// The one-question door onto this data is `make site X=docs Q="..."` (harness/author/docs.mjs).
//
...
function indexBody(entries) {
  const out = [];
  out.push('_GENERATED by `make site X=doc-index` from the `when:` / `answers:` frontmatter on each doc. Do not edit._');
  out.push('');
...
// sets, and a generator cannot trim what only grows. `make site X=docs Q="<question>"`
...
export function renderCraftBlock(entries) {
  ...
  out.push('_GENERATED by `make site X=doc-index` from each guide\'s `when:` / `answers:` frontmatter, the rows cannot');
  out.push('drift from the docs. Change a description in the doc, then run `make site X=doc-index`. The whole-repo map,');
...
export function staleViews(entries) {
  ...
  for (const [f, want] of views(entries)) {
    ...
    if (cur !== want) errs.push(`${f} is stale, run \`make site X=doc-index\``);
  }
  return errs;
}
