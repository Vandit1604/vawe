...
<!-- Generated by \`make scaffold\` (CONTINUOUS ACTION plan, ${dur}s < ${CONTINUOUS_ACTION_MAX_S}s: skills/vawe-continuous-action/SKILL.md).${sig ? ` Study formats/scene/${exemplar.file} (${exemplar.register || exemplar.teaches}) before writing.` : ''}
...
${states.map(stateSection).join('\n')}`;
  ...
  writeScaffold(scene, storyboard, [
    ...
    ...(sig ? [`  nearest proven film: ${exemplar.file} (${exemplar.register || exemplar.teaches}): study formats/scene/${exemplar.file}.`] : []),
  ]);
} else {
  // ==================================================================================================
...
<!-- Generated by \`make scaffold\` (BEAT ROTATION plan, ${dur}s >= ${CONTINUOUS_ACTION_MAX_S}s).${sig ? ` Study formats/scene/${exemplar.file} (${exemplar.register || exemplar.teaches}) before writing.` : ''}
     Write the film's prompt first (\`make ideate\`), then replace every \`REPLACE:\`/\`<fill: ...>\` marker,
     then \`make storyboard-check SB=${path.relative(ROOT, storyboardPathFor(out))}\`. -->
