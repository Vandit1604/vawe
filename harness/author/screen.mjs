...
// ---------------------------------------------------------------------------
// VIDEO-READINESS: what a fragment's own source can prove, without a render. Exported and pure (no
// I/O) so it is unit-testable on a string, separately from the CLI below (quality/gates/screen-readiness.test.mjs).
// ---------------------------------------------------------------------------
export function readiness(source) {
  const kit = extractKitBlock(source);
  const own = kit ? source.replace(kit, '') : source;

  // role -> px, read off the KIT's own ramp (the block this fragment pasted), so a role's size is
  // never guessed twice.
  const roleSizes = {};
...
function main() {
  ...
  // the real theme and screenshots 1920x1080. `--no-detect` here because step (b) below runs
  // impeccable's CLI detector directly on the source (its static-HTML engine), which needs no browser;
  // running the browser engine too would just print the same family of finding twice.
