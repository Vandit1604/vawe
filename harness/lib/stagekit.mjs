...
export const KIT_START = '/* STAGEKIT:start (generated by `make dev-tool X=stagekit`, DO NOT EDIT BY HAND, paste verbatim into every fragment) */';
 ...
 *   css:   the bare CSS rules (no <style> tag, no markers): what `make dev-tool X=stagekit` writes to a file.
...
export function kitCheck(goldenBlock, fragments) {
  ...
  for (const { path: p, src } of fragments) {
    ...
    if (got == null) { findings.push({ path: p, code: 'kit-missing', message: `${p}: no STAGEKIT block found. Paste the block from \`make dev-tool X=stagekit\` verbatim at the top of the fragment.` }); continue; }
    if (got !== goldenBlock) {
