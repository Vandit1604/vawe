...
holds: gated (make audit ASPECT=all; overlap/clipped-text/safe checks read core/safe.js DESTINATIONS)
...
two lines at the bottom (`CAPTION_LINES = 2` in `core/safe.js`), so a headline placed near the bottom
