...
`resolveEasing`, `scene.html:346`). `spring()` exists in `core/motion/motion.js` but isn't usable on the track, so
...
math is entangled with DOM writes and can't be unit-tested. `core/motion/motion.js` *is* tested (`lib-test`, 168
asserts).
...
`core/timeline/sequence.js` module of `(config, t) → value` functions; unit-test the table like `spring.test.ts`.
...
- Extract pure timeline math → `core/timeline/sequence.js` + unit tests (dim 6).
