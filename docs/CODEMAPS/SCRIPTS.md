...
USE it**, and it is three times the size of the engine it serves: 311 files, 59,823 lines.
...
| `scripts/author` | 82 | 15194 | WRITE. Everything that produces or edits a film: the brief, the storyboard, the stage kit, the fragments' briefs, assemble, the motion director. |
...
| `scripts/media` | 26 | 2903 | CAPTURE. Real pixels from the outside world: screenshots of live sites, video probes, image work. |
...
| `scripts/fonts` | 2 | 320 | FONTS. Fetch and bake typefaces. |
| `scripts/live/test` | 4 | 270 | Tests for the hooks. |
| `scripts/sim` | 2 | 178 | SIMULATE. Bake a physics sim to frames. |
| `scripts/hooks` | 1 | 131 | One hook helper. |
| `scripts/ransom` | 1 | 130 | Bake the ransom-note letter pack. |
...
| `lightfield-test.mjs` | 397 | yes | scripts/author/lightfield-test.mjs: the contract, as runnable assertions. |
...
| `lightfield-metrics.mjs` | 263 | no | scripts/author/lightfield-metrics.mjs: the measurements, defined once. |
| `build-refstudy.mjs` | 260 | no | scripts/author/build-refstudy.mjs: a shot-for-shot study of refs/pin-522769469268499616.mp4. |
| `lightfield-model.mjs` | 256 | no | scripts/author/lightfield-model.mjs: the colour field, evaluated on the CPU, and the palette |
...
| `lightfield-fit.mjs` | 193 | no | scripts/author/lightfield-fit.mjs: find the option set that best matches a reference. |
...
| `lightfield-compare.mjs` | 141 | yes | scripts/author/lightfield-compare.mjs: measure a generated field against the reference. |
...
| `lightfield-render.mjs` | 107 | no | scripts/author/lightfield-render.mjs, take a screenshot that is the same picture every time. |
...
| `lightfield-check.mjs` | 97 | no | scripts/author/lightfield-check.mjs: one row per LOOK, not one averaged number. |
...
| `lightfield-model-check.mjs` | 69 | yes | scripts/author/lightfield-model-check.mjs: is lightfield-model.mjs a model of the field, or just |
| `build-orbit.mjs` | 68 | no | scripts/author/build-orbit.mjs: generate formats/scene/orbit-proof.json. |
| `lightfield-seeds.mjs` | 64 | no | scripts/author/lightfield-seeds.mjs: search the seed space for the layout closest to a reference. |
...
| `lightfield-shot.mjs` | 45 | no | scripts/author/lightfield-shot.mjs: screenshot a generated field at a fixed frame. |
...
| `effects-catalog.mjs` | 254 | yes | scripts/site/effects-catalog.mjs, GENERATE docs/EFFECTS.md: the single arsenal catalog so the model |
...
| `vocab-catalog.mjs` | 131 | yes | scripts/site/vocab-catalog.mjs: regenerate docs/CRAFT/VOCABULARY.md from core/vocab.js. |
...
| `blocks-docs.mjs` | 36 | yes | scripts/site/blocks-docs.mjs: regenerate the block table in docs/BLOCKS.md from the manifest, so the docs |
| `deck.mjs` | 34 | yes | scripts/site/deck.mjs: publish docs/animation.html to the marketing site as /deck. |
...
| `fonts.mjs` | 136 | yes | Fetch the FREE, openly-licensed faces the engine registers in core/tokens.css into the |
...
| `geo-bake.mjs` | 115 | no | scripts/media/geo-bake.mjs: regenerate assets/geo/us-states.js and assets/geo/world.js. |
...
| `audio-bake.mjs` | 103 | yes | audio-bake.mjs: bake every sound the engine can use, from parameters, with no network. |
...
| `gradients.mjs` | 80 | yes | scripts/media/gradients.mjs: DEPRECATED. bake a gradient-background pack into a render-ready library. |
...
| `globe-dots.mjs` | 76 | yes | scripts/media/globe-dots.mjs: bake the world into a list of points, once, offline. |
| `cutout.mjs` | 63 | yes | scripts/media/cutout.mjs. A photograph becomes a PROP: background removed, alpha kept. |
| `watermark.mjs` | 62 | yes | scripts/media/watermark.mjs: bake the DRAFT watermark to a transparent PNG. |
...
FONTS. Fetch and bake typefaces.

| file | lines | make | what it says it does |
|---|---|---|---|
| `glyphs.mjs` | 201 | yes | glyphs.mjs: turn a vendored woff2 into a three.js "typeface JSON" of glyph OUTLINES. |
| `verify-render.mjs` | 119 | yes | verify-render.mjs: prove a generated typeface JSON actually EXTRUDES, in the right typeface. |
...
SIMULATE. Bake a physics sim to frames.

| file | lines | make | what it says it does |
|---|---|---|---|
| `run.mjs` | 134 | yes | scripts/sim/run.mjs: bake a stateful simulation to a deterministic PNG frame sequence. |
| `provenance.mjs` | 44 | no | scripts/sim/provenance.mjs: what produced a bake, as a hash. |
...
Bake the ransom-note letter pack.

| file | lines | make | what it says it does |
|---|---|---|---|
| `sprites.mjs` | 130 | yes | scripts/ransom/sprites.mjs: turn a pack of real cut-out letter images into a sprite set the |
