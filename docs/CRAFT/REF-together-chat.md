...
Compare `docs/CRAFT/CONTINUITY-WITHOUT-AN-OBJECT.md`: the other reference cut every 1.52s. This one
...
closing *U.S. & Canada*. Compare `docs/CRAFT/COLOR.md`. Restraint here is a measurement, not a mood.
...
`docs/CRAFT/CONTINUITY-WITHOUT-AN-OBJECT.md` asks for the register, not just the presence. This film runs
...
thing our engine has no primitive for.** `docs/MISTAKES.md` #59 records why per-layer `perspective()` was
...
| 2 | the studio **frame preview** at four or five scrub positions | frames pulled from a scene we already ship, via `make frame M=<name> N=<n>` | 1, our own render output | these are the pictures riding above the timeline in beats 3 and 5 |
