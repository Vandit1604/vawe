---
when: routed here by `docs/CRAFT/ROUTING.md`, or a request must faithfully reflect a real film or site with no product to sell
answers: "the recreation intake questions, its honesty ceiling, and which CRAFT docs it draws on"
group: crosscutting
---
...
- **Trigger:** "recreate this ad/film", "make ours look like this video", "match this site's design
  exactly" with no product of ours to sell. If the goal is to market a product using a captured site,
  that is `launch-video.md`; recreation has no separate commercial subject, the reference IS the
  subject.
...
- **Honesty check**: name up front which parts cannot be matched exactly, so the sign-off is against
  the real ceiling, not an imagined perfect copy.

## Blueprint family + docs

- Follow `docs/CRAFT/RECREATION.md`'s ordered loop: measure -> capture -> build -> score -> beat-sync
  -> verify. One beat per section, in the reference's own order.
- `docs/CRAFT/REFERENCE-STUDY.md` for a film reference: shot boundaries, in/mid/out frames, the four
...
- Blueprint family: whatever the reference itself uses; do not default to `kineticHook`/`statReveal`
  rotation here, the reference dictates structure.
