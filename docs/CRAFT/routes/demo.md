---
when: routed here by `docs/CRAFT/ROUTING.md`, or a request wants to prove one mechanism works, not ship a film
...
- **The subject must be a picture**, not a blank rectangle: a debug-harness grid with nothing pictorial
  in it is the failure mode `docs/CRAFT/SPECIMEN.md` exists to prevent.
...
- **`make demo Q="…" [NAME=…] [FX=…] [SUBJECT=…]`** writes the archetype and runs the dev loop
  directly; read `docs/CRAFT/SPECIMEN.md` first for why the subject is fixed and every constant is
