...
group: look
codes: buried, clipped-component, clipped-text, collapsed-image, degenerate-pin, overflow, overlap, safe
applies-when: always
...
- **Inside a hand-authored `html` fragment**, the same grid exists as CSS: `make stagekit` generates
  ...
  instead of cards. [HTML-FRAGMENTS.md](HTML-FRAGMENTS.md) has the full kit; four worked fragments, one
  per archetype below, are in [FRAGMENT-EXEMPLARS.md](FRAGMENT-EXEMPLARS.md).
