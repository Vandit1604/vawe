...
loud one. That refusal is gone now, not the discipline behind it: `core/expand.js`'s `expandScene`/
...
and no error. It now bakes at boot (`bakeCameraMove` in `core/produce.js`, also called from
`core/expand.js` for the Node consumers that never reach `core/boot.js`), and `core/boot.js` THROWS if
