---
when: writing or reading a docs/CRAFT/rules/<category>.json file, or wiring a new consumer of harness/lib/craft-rules.mjs
answers: "the rule-record schema, one worked example, the owner rules every record obeys, and why brief quotes the doc"
group: crosscutting
---
# docs/CRAFT/rules: one record per rule, one file per category

Four ways this repo already said "follow this rule" (docs/RULES per-rule files, a doc's own
...
```json
{
  ...
  "doc": "docs/RULES/ease-direction.md"
}
```
...
| `doc` | `docs/....md` or `docs/....md#anchor`; the file (and the anchor's heading, if given) must exist |
...
- **Uniformity lives in the film's own `design.md`**, never in a rule record. A record points an author
  at the mechanism (`docs/CRAFT/THEME-LOOK.md`, `<film>.design.md`); it does not re-derive the numbers.
