...
**Seen in.** A reference tutorial whose whole thesis is animating light to control where a viewer looks. Cross-checked against docs/CRAFT/EYE-TRACE.md, which ranks the eye's targets as brighter > larger > in focus > moving and treats every one of them as a STATIC property measured at a cut.
...
**The fix.** FIXED, all four. `--glow-c` crossfades a light's colour; `--plane-z` travels a layer in Z so it grows by approaching rather than by scaling; camera `f`/`a` rack the focus through the depth planes and every layer reacts by where it stands; and a `beam` can now feed a `matte`, so light reveals what it touches. docs/CRAFT/EYE-TRACE.md gained the section that says which knob moves which term.
