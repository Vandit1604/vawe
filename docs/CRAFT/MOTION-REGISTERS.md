...
- `docs/CRAFT/DIRECTION.md` §3 and `docs/CRAFT/TASTE-RULES.md` state restraint as a universal law: name
...
sequence with no still beat at all. `docs/CRAFT/DIRECTION.md` §1 already cites `higgsfield-recreation`
...
to search for by name. Naming them is the fix `docs/CRAFT/HTML-FRAGMENTS.md`'s
...
`docs/CRAFT/DIRECTION.md`'s own pacing rules (accelerate toward the climax, alternate short/long beats)
...
`docs/CRAFT/FILM-STRUCTURE.md` already treats several structural devices (a motif, a sound bridge, an
...
REPO, never off an external reference video or a named outside brand. `docs/CRAFT/HTML-FRAGMENTS.md`'s
...
- Saul Bass's title sequences and Kyle Cooper's *Se7en* main title (1995): the continuous-kinetic title tradition, cited already in `docs/CRAFT/FILM-STRUCTURE.md`.
