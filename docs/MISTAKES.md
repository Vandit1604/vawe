...
committed presets rather than by taste.

**Hue.** Every role's hue was rolled independently over the whole circle. Measured, the LIT roles of the
five presets span an arc of 10, 22, 39 and 40 degrees. The one outlier is `fern` at 86, and `fern` is
one of the two looks that was judged bad by eye. So a palette is ONE hue with offsets: a roll moves the
anchor while every role keeps its offset, its lightness and roughly its saturation. Narrow is not a
taste being imposed, it is what everything that already works here does.

**Contrast.** Rolling `shadow` alone took colonnade's near-black from 23% of the frame to **3.2%**,
while rolling `colour` alone left it at 25.1%. The cause is SIGNS: `sheen: -1` means an emitted
silhouette and `seam: -0.6` means a bright hairline, so a nudge crossing zero does not vary the picture,
it deletes the thing the picture is made of. A nudge now stays on its own side of zero, and the same
measurement reads 42.3% against a 42.7% baseline.

**The rule all of this is an instance of.** A randomiser preserves the COMPOSITION and rolls the anchor.
Lightness ordering, hue shape, and the sign of a contrast dial are all composition. What is left to roll
is which colour it is and where the light sits, which is variation. Scrambling every axis at once is not
...
**CLOSED by #425.** This entry also said `motion-audit`'s single inferred window may be a bad rule, and
...
and the gate was also scoring the CAMERA RIG as content. See #425.

## #425 — the motion contract's windows came from a field nobody writes, and one of its clauses was measuring the camera
