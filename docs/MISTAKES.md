...
`tools/lightfield/lightfield-shot.mjs` gives one: a stage at the exact output size with `--t` set explicitly.
holds: none

## 272. a fidelity metric that averages away the thing it is grading
`tools/lightfield/lightfield-metrics.mjs` now defines both, once, and both are printed: `blockError` for the colour field, `striping` for the pattern (`edge`, the mean absolute horizontal step;...
holds: none
...
`stableShot` (`tools/lightfield/lightfield-render.mjs`) shoots until two consecutive frames are byte-identical, and throws when that never happens.
holds: none
...
`tools/lightfield/lightfield-fit.mjs` imported `open`, `W` and `H` from `lightfield-render.mjs`, which exported none of them, so the tool could not run at all and nothing said so.
...
`tools/lightfield/lightfield-seeds.mjs` cannot run: it imports `tools/lightfield/lightfield-model.mjs`, which is not in the repository.
holds: none
