...
blinking caret, then a cursor clicks the send button (a black circle, arrow icon). [use: cursor]
...
once the sentence completes; faint grey triangle/circle/square motifs drift in the background [use: deep-ocean]
...
shift under each card (pale blue-grey, then olive, then brick red) as if lit by the photo on screen [use: arctic-blue]
...
blue then olive then red as the cards change, not a flat fill [use: glitchGlow]
...
circle motifs sit in the background, on plain light grey [use: plain]
...
lamp, a pendant, styled shots) fills in beneath it, ending at six photos in two rows [use: colorWave]
...
ground: mid, olive, #8c7c68 (measured, luma 175.6); an olive-brown wash behind the tilted white window [use: fadedPolaroid]
...
wordmark and hold, centred, on plain light ground [use: charOvershoot]
