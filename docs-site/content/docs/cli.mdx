...
The module is read from the JSON's `"module"` field, and the output defaults to `engine/out/<name>.mp4`. Flags may follow the file.

| Flag | Purpose |
|---|---|
| `--data` | The scene JSON (also accepted as a positional argument). |
| `--out` | Output path. Defaults to `engine/out/<name>.mp4`. |
...
make beats D=formats/scene/my.json      # first, mid, and last frame of every beat → /tmp/beats.png
make scrub F=engine/out/my.mp4          # a contact sheet of the whole film
```
...
The full list of targets is in the repo's [`Makefile`](https://github.com/Vandit1604/vawe/blob/main/Makefile). Every quality check has its own wrapper too, covered in [Quality gates](/docs/quality-gates).
