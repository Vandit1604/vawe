...
A strength rides on the name (`"neon:0.6"`), and `lookOpts` overrides properties (many looks default their colour to the brand accent, so a look reskins per theme). Reach for one look per beat, on real content, never as decoration on empty space.
