...
This downloads the curated set into `assets/sfx/` and records attribution to `credits.json`. The library: `whoosh`, `reveal`, `swoosh`, `pop`, `tick`, `click`, `impact`, `riser`, `beep`, `beep3`, `correct`, `wrong`. `whoosh` covers cuts and `reveal` covers stings, which is what the automatic mode reaches for. The sounds are fetched, never committed.
