...
- `split` (`char`, `word`, or `line`) breaks the text into units, and `preset` animates each one. See the [25 kinetic presets](/docs/motion#kinetic-typography). `each` is the per-unit duration, `stagger` the delay between units.
