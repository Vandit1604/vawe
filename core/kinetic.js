...
// splitText(el, mode): wrap each char|word|line of el.textContent in a <span class="ku"> so units
// animate independently. Returns the unit spans (in order). Idempotent-ish: call once at build.
// Preserves spaces; 'word' keeps words unbreakable. HTML in el is flattened to text.
export function splitText(el, mode = 'word') {
  ...
  if (mode === 'char') {
    ...
    text.split('\n').forEach((ln, i) => { if (i) el.appendChild(document.createElement('br')); const s = mk(ln); el.appendChild(s); units.push(s); });
  } else { // word
    const parts = text.split(/(\s+)/); // keep the whitespace tokens
    for (const p of parts) { if (p === '') continue; const s = mk(p); el.appendChild(s); if (p.trim()) units.push(s); }
  }
  return units;
}
...
export const PRESETS = {
  ...
  up: (u, { dist = 40 } = {}) => ({ opacity: clamp01(u), transform: `translateY(${((1 - easeOutCubic(u)) * dist).toFixed(1)}px)` }),
  down: (u, { dist = 40 } = {}) => ({ opacity: clamp01(u), transform: `translateY(${(-(1 - easeOutCubic(u)) * dist).toFixed(1)}px)` }),
  ...
  // blur + fade in
  blur: (u, { px = 16 } = {}) => ({ opacity: clamp01(u), filter: `blur(${((1 - easeOutCubic(u)) * px).toFixed(1)}px)` }),
  // springy bounce in
  bounce: (u, { bounce = 0.5, settle = 0.5, dist = 60 } = {}) => { const s = spring(u * settle * 2, { bounce, settle }); return { opacity: clamp01(u * 3), transform: `translateY(${((1 - s) * dist).toFixed(1)}px)` }; },
  // slide from a side
  slide: (u, { dir = 'left', dist = 80 } = {}) => { const k = 1 - easeOutCubic(u); const x = (dir === 'left' ? -1 : dir === 'right' ? 1 : 0) * k * dist; const y = (dir === 'up' ? -1 : dir === 'down' ? 1 : 0) * k * dist; return { opacity: clamp01(u), transform: `translate(${x.toFixed(1)}px, ${y.toFixed(1)}px)` }; },
  // persistent sinusoidal wave (u is used as raw phase, not a one-shot) — pass loop:true in animateUnits
  wave: (u, { amp = 14, phase = 0 } = {}) => ({ opacity: 1, transform: `translateY(${(Math.sin(u * Math.PI * 2 + phase) * amp).toFixed(1)}px)` }),
};
