...
export function build(kit, el, L, { cols: colData, w, gap, headSize, cardSize } = L) {
  const cols = (colData || []).slice(0, 4);
  const colW = ((w ?? 1200) - (cols.length - 1) * (gap ?? 28)) / Math.max(1, cols.length);
  ...
  el.style.display = 'flex'; el.style.gap = (gap ?? 28) + 'px';
  el.innerHTML = cols.map((c) => `
    <div style="width:${colW.toFixed(1)}px;display:flex;flex-direction:column;gap:14px">
      <div style="display:flex;gap:12px;align-items:baseline;font:600 ${headSize ?? 24}px var(--font-sans);color:var(--text)">${c.title || ''}<span style="font:500 ${(headSize ?? 24) - 4}px var(--font-mono);color:var(--dim)">${c.count ?? ''}</span></div>
      ${(c.cards || []).slice(0, 5).map((k) => `
        <div style="background:var(--surface);border-radius:10px;padding:18px 20px;box-shadow:${ring}">
          <div style="font:500 ${cardSize ?? 19}px var(--font-mono);color:var(--dim)">${k.id || ''}</div>
          <div style="font:510 ${(cardSize ?? 19) + 3}px var(--font-sans);color:var(--text-2);margin-top:6px">${k.title || ''}</div>
          ${(k.labels || []).length ? `<div style="display:flex;gap:10px;margin-top:12px">${k.labels.map((lb) => `<span style="font:500 ${(cardSize ?? 19) - 3}px var(--font-sans);color:var(--text-2);background:rgba(255,255,255,0.04);border-radius:5px;padding:3px 10px;box-shadow:inset 0 0 0 1px rgba(255,255,255,0.06)"><span style="display:inline-block;width:7px;height:7px;border-radius:50%;background:${lb.c || '#8a8f98'};margin-right:7px;vertical-align:1px"></span>${lb.t}</span>`).join('')}</div>` : ''}
        </div>`).join('')}
    </div>`).join('');
}
