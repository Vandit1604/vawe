// core/expand.js: expandScene(data), the PURE data->data expansion of the three build-time sugar layer
 ...
 * already baked, e.g. by core/boot.js for the browser render path), and strips the top-level `comps`
...
export function expandScene(data) {
  ...
  // cameraMove sugar -> data.camera. ONE implementation, core/produce.js. Idempotent (it deletes the
  // field), so calling it here after core/boot.js already baked it for a browser render is a no-op.
