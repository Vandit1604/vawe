...
export const KNOBS = {
  ...
  three: {
    ...
    // litPlane reads a real capture and refuses an undecoded one (core/surfaces/three-fx.js textureFrom), so
    // `screen` has no useful default. Every value below is the scene's own `??` default, not a guess.
    litPlane: [{ name: 'screen', type: 'string', default: '', desc: 'the captured UI image on the plane (required)' }, n('rise', 0, 'resting height the plane rises to'), n('pitch', 0.08, 'resting tilt'), n('yaw', -0.06, 'resting turn'), n('travel', 0.7, 'how far back it pulls while arriving'), n('roughness', 0.42, 'surface roughness'), n('metalness', 0.04, 'metalness'), col('bodyColor', '#d9cdb9', 'ground colour')],
...
export const SPECTACLE_DEVICES = defineRegistry('spectacle device',
  ...
  catalog: {
    ...
    usage: (name, { j }) => j({ spectacle: { at: 2.4, of: 'hero', device: name, why: 'the one loud moment, and every other dial drops to 55%' } }),
