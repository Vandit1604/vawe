...
// typo deserves the same near-word hint every other named vocabulary gets (core/registry.js), rather
...
// that lookup and its near-word hint). `nearMisses` is core/registry.js's own helper; when omitted, a
...
// by `make validate`) hands in every list; `core/boot.js`'s browser-side theme check does too, since
...
// the browser loads: see core/validate.mjs's CLI branch for where it IS checked.
