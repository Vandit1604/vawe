...
function validateBridgeSound(b, bad) {
  const kind = b.bridge;
  if (kind !== 'j' && kind !== 'l')
...
function resolveJunction(b, table, bad) {
  ...
  return { ref, at: times[idx] };
}

function jSpan(b, at, bounds, ref, bad) {
  const { prev, next, duration } = bounds;
  const lead = +(b.lead ?? 0);
  ...
  return { start: at - lead, end: at + span };
}

function lSpan(b, at, bounds, ref, bad) {
  const { prev, next } = bounds;
  const lag = +(b.lag ?? 0);
  ...
  return { start: at - span, end: at + lag };
}
...
function fadeGainDuck(b, start, end, bad) {
...
export function resolveBridges(audio, marks, duration) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    const kind = validateBridgeSound(b, bad);
    const { ref, at } = resolveJunction(b, table, bad);
    ...
    const bounds = { prev, next, duration };
    const { start, end } = kind === 'j'
      ? jSpan(b, at, bounds, ref, bad)
      : lSpan(b, at, bounds, ref, bad);

    const { fade, gain, duck } = fadeGainDuck(b, start, end, bad);
