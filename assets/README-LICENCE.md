...
`make gen X=audio` (`core/audio/kit.mjs`: noise, a biquad, an envelope, seeded). Same parameters always give
...
**`assets/music/*.wav`: DOWNLOADED third-party tracks.** `make gen X=music` / `make gen X=music-pack`
...
> one thing the licence forbids. A fresh clone has no beds; `make gen X=music-pack` refills them.
...
- **A fresh clone renders every film silent.** A scene naming `assets/music/lofi.wav` finds nothing,
  ...
  `bed-missing`; run `make gen X=audio` and `make gen X=music-pack` before rendering anything with a bed.
...
(`make gen X=gradients`, `make gen X=ransom-sprites`) are the durable part; the images can always be re-baked.
