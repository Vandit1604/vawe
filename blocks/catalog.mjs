...
export const CATALOG = [
  ...
  { name: 'splitFlapBoard', family: 'splitFlapBoard', blurb: 'a mechanical departure board · a drum per character steps through the alphabet and lands',
  ...
  { name: 'nowPlaying', family: 'nowPlaying', blurb: 'music-player card · artwork, progress, transport',
    props: {"w": 380, "track": "Golden Hour", "artist": "Field Notes", "progress": 0.62} },
  { name: 'videoLowerThird', family: 'videoLowerThird', blurb: 'creator lower third · avatar, subs, red CTA',
  ...
  { name: 'glassNotification', family: 'glassNotification', blurb: 'frosted alerts fly in from the right and stack, each card narrower than the one above it',
    props: { w: 520, items: [
  ...
  { name: 'glassMenu', family: 'glassMenu', blurb: 'frosted command panel: icon column, rows, separators, one row lit with the accent',
  ...
  { name: 'glassControls', family: 'glassControls', blurb: 'frosted media panels SPREAD out of one collapsed point: scrubber, transport, level meter',
    props: { w: 760, track: 'Deterministic render', elapsed: '01:12', total: '03:40', progress: 0.34, bars: 9 } },
  { name: 'glassHome', family: 'glassHome', blurb: 'frosted launcher grid led by one wide widget tile; tiles arrive on the diagonal',
  ...
  { name: 'glassDock', family: 'glassDock', blurb: 'floating frosted dock strip with one item magnified, its neighbours swelling toward it',
  ...
  { name: 'codeHighlight', family: 'codeHighlight', blurb: 'a band sweeps down to one line while the context dims out of the way',
    props: { w: 620, label: 'server.ts', theme: 'ink', line: 2, lines: [
      'export function serve(req) {', '  const t = route(req.url);', '  return t.handle(req);', '}'] } },
  { name: 'codeScroll', family: 'codeScroll', blurb: 'a viewport onto a long file: it scrolls until the target line centres, then lights it',
  ...
  { name: 'codeDiff', family: 'codeDiff', blurb: 'the edit PERFORMED: removals collapse red, additions expand green, in edit order',
  ...
  { name: 'codeMorph', family: 'codeMorph', blurb: 'a refactor as a transformation: shared tokens glide, the rest fade out and in',
    props: { w: 620, label: 'refactor', theme: 'dusk',
      from: ['const out = items.filter(fn);', 'return out;'], to: ['return items.filter(fn);'] } },
  { name: 'codeFlight', family: 'codeFlight', blurb: 'discrete snippets fly in from alternating sides and assemble into one program',
  ...
  { name: 'worldMap', family: 'worldMap', blurb: 'world choropleth by ISO alpha-3 (Natural Earth I), data countries arrive over a live base map',
  ...
  { name: 'usMapBubble', family: 'usMapBubble', blurb: 'proportional circles at city coordinates (AREA is the value) with name + value callouts',
  ...
  { name: 'parallaxZoom', family: 'parallaxZoom', blurb: 'one card eats the frame: the centre cell scales to fill the board while its eight neighbours travel outward and dim',
  ...
  { name: 'parallaxUnzoom', family: 'parallaxUnzoom', blurb: 'the same board run backwards: the hero starts filling the frame and retreats into its cell as the ring arrives',
  ...
  { name: 'redditPost', family: 'redditPost', blurb: 'a link-aggregator post: the vote rail left of the title, subreddit line, body, comment count',
  ...
  { name: 'uiReveal3d', family: 'uiReveal3d', blurb: 'UI rows folding up out of depth: each hinges at its top edge in real perspective, one after another',
