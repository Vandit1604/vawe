...
export * from './app.mjs';
// charts.mjs — the plot family (bars/lines/donut/stack/gauge/ring + the two stat blocks). Extracted so
// index.mjs stays the assembly point, not an ever-growing wall (same pattern as app.mjs / interact.mjs).
import * as CHARTS from './charts.mjs';
export * from './charts.mjs';
// dev.mjs (code/terminal/deploy/log/diff/file/spinner), social.mjs (chat/tweet/avatar/social/media),
// ui.mjs (frames/tabs/tables/lists/chrome) — the same extraction, so index.mjs is the assembly point,
// not a 1500-line wall. Each shares the kit vocabulary and is merged into the registry below.
...
// What remains here: the plain cards/statements (card/quote/stripeCard/kpiRow/comparison/pricingCard/
// captions/colorCycle), the big variant families (lowerThird/searchEngine), and the COMPOSITION blocks
// (splitScreen/screenSwap) that read the BLOCKS registry itself. Everything else lives in a sibling.
const FACTORIES = { ...APP, ...CHARTS, ...DEV, ...SOCIAL, ...UI,
  card, colorCycle, stripeCard, quote, kpiRow, comparison, captions, pricingCard,
  lowerThird, searchEngine, splitScreen, screenSwap };
