...
study-tool X=palette` call. `hooks/` is the git-hook library (`library.mjs`). `site/` builds artifacts the
...
The one doc: none single; `AGENTS.md` names `make sections`/`make study-tool X=palette` for the brand-reflection
