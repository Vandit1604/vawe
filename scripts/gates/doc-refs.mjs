 ...
 */
function selfRefs() {
  const RUN = /\bnode\s+((?:scripts|core|verify|blocks)\/[A-Za-z0-9_./-]+\.(?:mjs|js))/g;
  const QUOTED = /['"`]((?:scripts|verify|blocks)\/[A-Za-z0-9_./-]+\.(?:mjs|js))['"`]/g;
  const out = [];
...
function makeRecipes() {
  ...
  mk.split('\n').forEach((line, i) => {
    ...
    if (!target) return;
    for (const m of line.matchAll(/\bnode\s+((?:scripts|core|verify|blocks)\/[A-Za-z0-9_./-]+\.(?:mjs|js))/g)) {
