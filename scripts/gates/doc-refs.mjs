...
//   * A path resolves if it exists relative to the repo root OR relative to the citing doc. Both
//     conventions are in use here, and a gate may not pick a favourite.
// Pure: reads files and the Makefile. No render, no network, no dependency.
 ...
 */
function selfRefs() {
  const RUN = /\bnode\s+((?:scripts|harness|generators|research|core|verify|blocks|tools|scene|films)\/[A-Za-z0-9_./-]+\.(?:mjs|js))/g;
  const QUOTED = /['"`]((?:scripts|harness|generators|research|verify|blocks|tools|scene|films)\/[A-Za-z0-9_./-]+\.(?:mjs|js))['"`]/g;
  const out = [];
...
function makeRecipes() {
  ...
  mk.split('\n').forEach((line, i) => {
    ...
    if (!target) return;
    for (const m of line.matchAll(/\bnode\s+((?:scripts|harness|generators|research|core|verify|blocks|tools|scene|films)\/[A-Za-z0-9_./-]+\.(?:mjs|js))/g)) {
...
function repoRoots() {
  const ls = cp.execSync('git ls-files', { cwd: ROOT }).toString().trim().split('\n');
  return new Set(ls.map((f) => f.split('/')[0]));
}
...
/** A path resolves if it exists from the repo root or from the citing doc's directory. */
function resolves(rel, fromDoc) {
  const bases = [ROOT, path.dirname(path.join(ROOT, fromDoc))];
  return bases.some((b) => {
    const abs = path.resolve(b, rel);
    return abs.startsWith(ROOT) && fs.existsSync(abs);
  });
}
  ...
  for (const rel of docs()) {
    ...
    for (const { text, line } of codeChunks(src)) {
      ...
      for (const p of pathsIn(text, roots)) {
        if (!resolves(p, rel) && !waived.has(p)) badPaths.push({ rel, line, ref: p, text: text.trim().slice(0, 90) });
      }
    }
    src.split('\n').forEach((line, i) => {
      for (const p of linkPathsIn(line, roots)) {
        if (!resolves(p, rel) && !waived.has(p)) badPaths.push({ rel, line: i + 1, ref: p, text: line.trim().slice(0, 90) });
      }
    });
  }
