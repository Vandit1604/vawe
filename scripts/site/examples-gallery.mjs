...
const REG = path.join(repoRoot, 'formats/scene/examples.json');
...
const card = (it) => `
  <figure class="card ${it.orientation}" data-poster="${it.poster ?? 0}">
    ...
    <figcaption>
      ...
      <a class="src" href="../../formats/scene/${esc(it.source)}">view source &rarr;</a>
    </figcaption>
  </figure>`;
...
<body>
  <div class="wrap">
    ...
    <table class="usecase">
      ${useCases.map((it) => `<tr><td>${esc(it.bestFor)}</td><td><a href="../../formats/scene/${esc(it.source)}">${esc(it.title)}</a> &middot; ${esc(it.description)}</td></tr>`).join('\n      ')}
    </table>
    ...
    <footer>
      Generated by <code>make gallery</code> from <code>formats/scene/examples.json</code>. Each card links to its scene source. Snippets: <code>engine-doctrine/MOTION-SNIPPETS.md</code>.
    </footer>
  </div>
<script>
