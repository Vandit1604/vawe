...
for (const f of files) {
  ...
  const want = JSON.stringify(strip(JSON.parse(fs.readFileSync(src, 'utf8'))), null, 1) + '\n';
...
if (!drifted.length) { console.log(`✓ ${files.length} site scene(s) match their source (author-only keys dropped: ${AUTHOR_ONLY.join(', ')})`); process.exit(orphans.length ? 1 : 0); }
