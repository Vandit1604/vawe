...
import { expandScene } from '../../core/expand.js';
...
// `block`/`beat`/`comp` sugar is expanded HERE, at publish time (core/expand.js `expandScene`, pure
// Node, same function `./bin/vawe` resolves it with server-side), so the picker's scenes always boot:
// the render page (`formats/scene/scene.js`) deliberately never imports core/expand.js itself (its own
