...
//   node scripts/site/effects-catalog.mjs [--check]   ·   make regen
...
// dedicated doc (`make regen` → engine-doctrine/CRAFT/VOCABULARY.md), generated from the same registry.
...
function emit() {
  ...
  out.push('');
  out.push('> GENERATED by `scripts/site/effects-catalog.mjs` (`make regen`) from the engine registries, it');
  ...
  out.push('| A whole beat, directed | a `spine` recipe (`recipes/README.md`) |');
  ...
  out.push(`---`);
  out.push(`_${total} effects across ${sections.length} families. Regenerate: \`make regen\`._`);
  const md = out.join('\n') + '\n';

  const dest = path.join(root, 'engine-doctrine/EFFECTS.md');
  if (CHECK) {
    const cur = fs.existsSync(dest) ? fs.readFileSync(dest, 'utf8') : '';
    if (cur.trim() !== md.trim()) { console.error('✗ engine-doctrine/EFFECTS.md is stale, run `make regen` (a registered effect is missing/changed).'); process.exit(1); }
