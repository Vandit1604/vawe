...
//   node scripts/site/vocab-catalog.mjs --check    fail if either doc is stale (make check GATE=generated-check)
//
...
out.push('');
out.push('> GENERATED by `scripts/site/vocab-catalog.mjs` (`make regen`) from `core/registry/vocab.js`. Do not edit.');
out.push('');
...
out.push('');
out.push('---');
out.push(`_${total} words across ${FAMILIES.length} families. Regenerate: \`make regen\`. The full engine`);
out.push('vocabularies these alias: `engine-doctrine/EFFECTS.md` (`make regen`)._');
...
function writeOrCheck(dest, content, label) {
  if (CHECK) {
    ...
    if (cur.trim() !== content.trim()) { console.error(`✗ ${dest} is stale, run \`make regen\`.`); ok = false; }
...
pout.push('');
pout.push('> GENERATED by `scripts/site/vocab-catalog.mjs` (`make regen`) from `registries()` in `core/registry/registry.js`. Do not edit.');
pout.push('');
...
pout.push('Regenerate: `make regen`. Ratchet: `make check GATE=word-action`._');
