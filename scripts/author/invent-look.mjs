...
//
//   node scripts/author/invent-look.mjs formats/scene/_concepts/gh-wrapped.storyboard.md --seed 7
//   node scripts/author/invent-look.mjs <storyboard.md> --seed 7 --pick 3 --name <theme>
...
if (!src) die('usage: node scripts/author/invent-look.mjs <storyboard.md> [--seed n] [--count 5] [--pick n --name theme] [--json]');
...
function buildTheme(c, name) {
  ...
  return {
    ...
    note: `INVENTED (not selected) by scripts/author/invent-look.mjs from ${path.relative(ROOT, file)}, seed ${seed}. Stance "${c.stance.key}": ${c.sentence}. Texture: ${c.texture}.`,
    palette: P,
    gradient,
    type: {
...
const MARK = '/* ---- invented faces (generated by scripts/author/invent-look.mjs) ---- */';
