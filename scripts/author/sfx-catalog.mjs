#!/usr/bin/env node
// scripts/author/sfx-catalog.mjs: generate docs/CRAFT/SFX-CATALOG.md from core/audio-kit.mjs's own
// CUES registry, so the catalog cannot drift from the engine. Run with `node scripts/author/sfx-catalog.mjs`.
//
...
function assertCoverage() {
  ...
  if (missing.length) {
    throw new Error(
      ...
      `Add each to META in scripts/author/sfx-catalog.mjs before regenerating the doc.`
    );
  }
...
- Enforced by \`make audio-check\` (code \`silence-without-a-reason\`): a film that ships \`audio.silent:true\`
  must say why. Regenerate this doc with \`node scripts/author/sfx-catalog.mjs\`; do not hand-edit it.
...
\`scripts/author/sfx-catalog.mjs\` from that same registry. Do not hand-edit it: edit the \`META\` map in
the script, then run \`node scripts/author/sfx-catalog.mjs\` to regenerate.
`;
}
