...
const ids = explicitId ? [explicitId] : await idsForGenre(genre);
if (!ids.length) { console.error(`no tracks found for genre "${genre}"`); process.exit(1); }
const id = ids[Math.min(rank, ids.length - 1)];

const mp3 = path.join(DEST, `${name}.mp3`);
const wav = path.join(DEST, `${name}.wav`);
const res = await fetch(`https://assets.mixkit.co/music/${id}/${id}.mp3`, { headers: UA });
if (!res.ok) { console.error(`track ${id} -> HTTP ${res.status}`); process.exit(1); }
const buf = Buffer.from(await res.arrayBuffer());
if (buf.length < 20000) { console.error(`track ${id} came back too small (${buf.length}b) — not audio`); process.exit(1); }
...
credits[name] = { id, genre: genre || 'direct', source: `https://assets.mixkit.co/music/${id}/${id}.mp3`,
...
console.log(`✓ ${name}.wav  (mixkit #${id}${genre ? ` · ${genre} #${rank}` : ''} · ${dur.toFixed(1)}s · ${(buf.length / 1024 / 1024).toFixed(1)}MB mp3)`);
