...
for (let p = 0; p < pages; p++) {
  const L = [{ type: 'text', text: `blocks · registry`, x: 70, y: 52, font: 'mono', size: 20, color: '#5C5C58', start: 0, duration: 9 },
    { type: 'text', text: `page ${p + 1}/${pages} · ${grid.length} blocks`, x: 1560, y: 52, font: 'mono', size: 18, color: '#9A9A94', start: 0, duration: 9 }];
  grid.slice(p * PER, p * PER + PER).forEach((e, i) => {
    const col = i % COLS, row = (i / COLS) | 0;
    const x = X0 + col * CELL_W, y = Y0 + row * CELL_H;
    L.push({ type: 'text', text: e.name, x, y: y - 26, font: 'mono', size: 18, color: '#0A0A0A', weight: 600, start: 0.1, duration: 9 });
  ...
  const scene = { module: 'scene', orientation: 'landscape', theme: 'shortwave-creed', duration: 9,
    audio: { silent: true }, bg: [{ preset: 'paper', from: 0, to: 9, value: 'light' }], layers: L };
