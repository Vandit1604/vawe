...
prefer real captured UI (`make media X=capture`) or a clean gradient over a generic photo.
holds: none
...
holds: quality/gates/conformance.mjs
...
holds: quality/gates/coverage.mjs
...
holds: quality/gates/conformance.mjs, quality/gates/lib-test.mjs
...
holds: quality/gates/author-check.mjs, quality/gates/blocks-audit.mjs, quality/gates/direction-floor.mjs, quality/gates/gate-mutation.mjs
...
## 33. Who checks the checkers ,  `make dev-tool X=gate-test`
...
holds: quality/gates/lib-test.mjs, quality/gates/probe-purity.mjs
...
the bar is 3:1 when an opaque sibling shape sits under the text (a structural test, not a colour heuristic), 7:1 otherwise. Gate: two `make dev-tool X=gate-test` cases, pinned in both directions ,  a grey...
holds: quality/gates/gate-mutation.mjs
...
the block forwards keyCue/keyGain onto the typing layer, AND `make dev-tool X=expand` now warns when a block layer carries a prop its factory does not accept ,  the parameter names are readable off the...
holds: quality/gates/blocks-audit.mjs, quality/gates/layer-props.mjs
...
holds: quality/gates/blocks-audit.mjs, quality/gates/sim-audit.mjs
...
holds: quality/gates/conformance.mjs, quality/gates/gate-mutation.mjs
...
holds: quality/gates/docs-drift.mjs
...
an extra positional argument is a hard error naming what was ignored and pointing at `make dev-tool X=compare`. ---.
holds: none
...
Mine, during the same session. I wanted the pre-change count for `make dev-tool X=gate-test`, so I stashed the tree, ran the harness, and popped.
holds: none
...
Building the `ransom` treatment, I checked determinism by rendering a frame 5× through `make dev-tool X=frame` / `harness/author/preview.mjs` and hashing the PNGs.
holds: none
...
## 112. `make media X=photos` wrote WebP bytes into files named `.jpg`
...
Both found while running the ladder on one film; neither is fixed in the gate that reports it. - `make dev-tool X=beats` reported "3 beats" for a film with seven distinct visual beats.
holds: none
...
Captured the habits + the study pipeline (measure → catalog motifs → map to primitives) in engine-doctrine/CRAFT/REFERENCE-STUDY.md, linked from CRAFT/README, with the reference-feel→primitive map and a...
holds: none
...
holds: quality/gates/audio-check.mjs
...
holds: quality/gates/schema-drift.mjs
...
`quality/gates/author-check.mjs` (`make dev-tool X=author-check`) chains validate · critique · direct · slop · inspect into one command, and `make video` runs it before rendering unless `NOCHECK=1`...
holds: none
...
Within `make dev-tool X=author-check`, a missing sidecar is surfaced as a visible WARN ("this scene declares no per-beat value contract"), and `--strict`/`STRICT=1` treats it as a failure.
holds: none

## 143. enforcement only gated the DOWNSIDE (slop); nothing forced ambition, so authoring stayed plain
Two-part forcing function: (1) `blueprints/` ,  directed-motion BEAT factories (`{type:"beat"}`, expanded by `make dev-tool X=expand`) so kinetic reveals / count-ups / cascades / dashboard dives are the...
holds: none
...
What. `make dev-tool X=beats` has always been step 2 of the authoring ladder, and it is the step that catches the things no static gate can see.
holds: none
...
holds: quality/gates/audit-scenes.mjs, quality/gates/lib-test.mjs, quality/gates/motion-audit.mjs
...
holds: quality/gates/beat-check.mjs, quality/gates/gate-mutation.mjs, quality/gates/lib-test.mjs
...
holds: quality/gates/scene-timing.mjs

## 173. the storyboard promised a change, the film never built one, and every gate stayed green
`quality/gates/plan-vs-render.mjs`, wired into `author-check` after `inspect` (it reads the same `.intent.json` sidecar, which already carried `span` and `becomes`) and standalone as `make...
holds: none
...
`quality/audit.mjs` reads `authoring.allow` the same way every other gate does.
holds: none
...
`quality/gates/visual-vocabulary.mjs`, blocking, wired into `author-check` beside the ambition floor and standalone as `make visuals`.
holds: none
...
`boxOf(L)` in `quality/gates/scene-timing.mjs` resolves size in five honest tiers: `explicit`, `size`, `intrinsic` (read the asset's own header, PNG IHDR / JPEG SOFn / SVG viewBox, no...
holds: none
...
`harness/dev/rules-audit.mjs` asks every gate finding the three questions the gates ask films: does it > Cut on 2026-08-05 in `cc2dfc2`. The audit below is the one run it produced.
holds: none
...
holds: quality/gates/lib-test.mjs, quality/gates/lint-test.mjs

## 217. the layout audit measured a rotating layer's empty corners, not the ink it draws
`quality/audit.mjs` samples SVG outlines for a true rotated bound, uses geometry ink on both axes, and CLAMPS the result to the border box.
holds: quality/gates/lib-test.mjs, quality/gates/lint-test.mjs
...
holds: quality/gates/plan-vs-render.mjs

## 220. the layout audit called a label on a card a collision
`quality/audit.mjs` now treats a layer as a SURFACE when it, or any descendant filling at least 85% of it, paints a background, and excludes surfaces from the text-overlap set.
holds: quality/gates/lib-test.mjs, quality/gates/lint-test.mjs
...
`quality/gates/snap-scenes.mjs` recycles the browser every 10 scenes.
holds: none

## 242. a factual finding was filed in the taste bucket, and a cull carried it out of sight
Moved to `quality/gates/beat-check.mjs`, which is always on, walks the clock rather than the layer list, and already owns `dead-air` ,  the same class of finding, the same reader.
holds: quality/gates/lint-test.mjs
...
holds: quality/gates/designspec-check.mjs
...
What happened. Wiring `impeccable`'s detector into `make preview` (approval stop 1b), the first result on every fragment and on `engine-doctrine/animation.html` was zero findings and exit 0.
holds: none
...
70 block families now declare what they accept (`blocks/schema.mjs`, gate `quality/gates/block-schema.mjs`).
...
holds: quality/gates/inspect.mjs

## 328. `make media X=assets` plans image cards for audio cue names
...
`quality/gates/arsenal-check.mjs`. It walks `core/`, imports each module, and treats an export as a vocabulary by its VALUE (an array of two or more strings, or an object keyed by names) rather...
...
holds: quality/gates/snap-signature.mjs
...
holds: quality/gates/unused.mjs
...
holds: quality/gates/direction-floor.mjs, quality/gates/lib-test.mjs, quality/gates/silent-fallback.mjs
...
holds: quality/gates/knobs-audit.mjs, quality/gates/lib-test.mjs
...
holds: quality/gates/lib-test.mjs, quality/gates/unused.mjs
...
holds: quality/gates/silent-fallback.mjs
...
`engine-doctrine/MOTION-CRAFT.md` recommended, twice, that "a value that should feel physical (number, bar, camera)" use `ease:"spring"`.
holds: quality/gates/snap-signature.mjs
...
holds: quality/gates/lib-test.mjs, quality/gates/plan-vs-render.mjs, quality/gates/schema-drift.mjs, quality/gates/snap-signature.mjs
...
## 402. `make dev-tool X=reveal` crashed on any single-beat film, and sampled every layer window a third too wide
...
`make dev-tool X=author-check` ran seven of its steps only when `TASTE=1` was set: `critique`, `direct`, `direction-floor`, `dissolve`, `designspec`, `copy`, `pace`.
holds: none
...
holds: quality/gates/snap-scenes.mjs
...
holds: quality/gates/snap-blocks.mjs
...
holds: quality/gates/site-counts.mjs
...
## 451. `make dev-tool X=beats --vs` printed a tick and wrote nothing (MISTAKES #245, half-fixed)
Both tools now adopt `openScene`, `scratch()` and `ffmpegOrDie`, and share one exported `drawtext()` escape in the new `harness/author/sheets.mjs`.
holds: quality/gates/gate-mutation.mjs
...
`quality/gates/motion-audit.mjs` scores five FAIL-tier clauses (i final-hold · ii monotonic · iii settle · iv count-up · v typing) against a WINDOW.
holds: quality/gates/motion-audit.mjs
...
`auditFrameFn` in `quality/audit.mjs` was 927 lines with a cyclomatic complexity of 166: the hardest function to change in this repo, and the gate that grades every film in the library.
holds: none
...
What. `make dev-tool X=author-check` expands `block` and `comp` sugar before it validates, and did not expand `beat`.
holds: none
...
holds: quality/gates/lib-test.mjs, quality/gates/prop-probe.mjs
...
What. A film about one thing, a refracting lens crossing a hard black horizon, hit this bug.
holds: quality/gates/lib-test.mjs
...
`harness/author/recency.mjs` answers one question, "did this name exist 14 days ago", by reading git.
holds: quality/gates/lib-test.mjs
...
the fix is verified end to end: a fresh scaffold, after `make dev-tool X=preflight D=<file>`, now passes `make dev-tool X=author-check` (exit 0).
holds: quality/gates/pace-check.mjs
...
holds: quality/gates/designspec-check.mjs (dead-token, inside author-check)
...
holds: quality/gates/legacy-fold.mjs (MANIFEST_LAST_COMMIT)
...
holds: films/scene/scene.js (`beatIsCurrent`, the per-layer clock clamp in `renderFrame`), make test
...
`core/timeline/sequence.js:134` declares `POSE`, the one table mapping an authored keyframe name to its pose slot and identity, and it carried x, y, scale, rot, opacity, blur, w, h, track, ox and oy. It did not carry `radius`, and that single omission is why every continuous object in this library could move and resize and never become a different shape. The reference film this repo argues from goes rectangle to pill to circle, measured frame by frame as `{w:39,h:22,radius:4}`, `{w:39,h:22,radius:11}` and `{w:22,h:22,radius:11}`: two of the three properties were already keyable, since a pill is a rectangle whose radius reached half its height and a circle is a pill whose width reached its height. So nothing was invented, a forgotten row was added. Identity is `null` rather than a number, following `w`/`h`/`track`, so a track that never mentions radius leaves the layer's authored corner exactly as it was instead of quietly writing one. `KEYFRAME_PROPS` is generated from `POSE`, so the boot-time refusal for stray keyframe keys covered the new name for free. The write goes through `chipBox` (core/layers/util.js), which already owned a layer's resting radius and whose own comment records the earlier bug where an absent radius silently rendered square. THE TRAP THAT MAKES THE OBVIOUS TEST WORTHLESS: a radius larger than half the box is legal CSS and clamps silently, so a track can interpolate through wrong values and report nothing, which is why the test asserts the RENDERED radius on real frames, not the authored number.
...
Update, 2026-09-26: the owner removed the approval stage and `/vawe-approve` outright. There is no sign-off step now; the draft render is where the owner looks and redirects, the same as HyperFrames or Remotion.
...
One correction, because the wrong reading is expensive: "a video frame is not a web page" is a claim about VIEWING CONDITIONS, not design quality. Web craft is the input, and inheriting it is why this engine renders HTML at all: `make media X=capture URL= SEL=` lifts a real component with its computed CSS into an animatable layer, and hand-authoring a fragment is the fallback for a surface that does not exist yet. What the medium breaks is a short list of decorative VALUES; hierarchy, restraint, rhythm, optical alignment and type pairing transfer unchanged. Transpose the decoration, keep the craft.
...
holds: harness/author/assemble.mjs, quality/gates/frame-check.mjs

## 605. a test that borrows a live film as its fixture fails the day someone approves that film
`tests/hooks/stage-gate.test.mjs` proved three denials against `vawe-oblique`, whose plan passed and which nobody had signed, a state a real film LEAVES. The day the user approved it, all three cases broke. The test survived its own trap only because it had asserted its precondition out loud ("the fixture film is unapproved, or these cases prove nothing"), so it failed clearly instead of passing vacuously, the only reason anyone noticed. A fixture must be BUILT and removed by the test, never borrowed from the working library, whenever the property under test is one the library can change. Assert the precondition anyway: it turns a silent hole into a loud failure.
holds: tests/hooks/stage-gate.test.mjs, tests/gates/stage.test.mjs

## 606. a gate keyed on the broadest promise convicts the films that keep it a different way
A new `frame-as-surface` finding fired when a storyboard claimed continuity and every beat was still a full-bleed frame. It keyed on `object:` OR `threads:`, and convicted `hinge`, which is authored correctly. Its `threads:` names a hand-drawn ink underline that draws on under one word per beat, plus a line of reasoning: a motif and an argument, both kept honestly INSIDE each beat's own fragment, neither needing a layer that travels. `threads:` answers "what holds the film across cuts"; a film under 15s hard-errors without one, so nearly every storyboard carries one and most promise something no positioned layer could satisfy. `object:` is the narrow promise, a thing that TRAVELS, and only it can be contradicted by frames that give it nowhere to go. Swept all 40 films with a storyboard and a scene: 1 fires, the film with the defect. Same shape as #603 and the second time in one session: a check written against the broad field, corrected by running it over the library BEFORE shipping it.
holds: quality/gates/frame-check.mjs

## 607. a worktree fan-out inherits the last COMMIT, not the tree you are looking at
Three agents were sent into git worktrees to change files still uncommitted in the main checkout. A worktree branches from HEAD, so one agent's worktree did not contain `quality/gates/frame-check.mjs` at all, and it spent a third of its run rebuilding a four-hundred-line file that already existed, from a copy in the parent's working tree. Another could not read the approved plan, because `.claude/` is gitignored. A third resolved `fragment:` against the film's own directory, a bug the parent caught only by re-running the check itself: every real storyboard writes that path relative to the repo root. COMMIT THE BASE FIRST, then fan out, and inline into the brief anything that lives in an ignored path. The parent's own verification is not ceremony: two of the three reports in that round were accurate and the third was confidently wrong in a way only a re-run could show.
holds: engine-doctrine/CRAFT/SUBAGENT-BUDGET.md, engine-doctrine/CRAFT/SUBAGENTS.md


## 608. a persistent object does not create motion, it only makes motion possible
The object path was built to fix films that read as slideshows, proved by three throwaway films differing only in structure. A, the old way: two beats, a fragment each, torn down and rebuilt at the cut. B: the same two beats sharing one fragment, so the DOM survives, one layer with `acrossBeats`. C: B plus a keyed object chain across the cut. The retired motion-floor gate (`engine-doctrine/SAFEGUARDS.md`) measured all three, same theme, same copy, same 6 seconds: A is 9 of 10 windows dead, B is 9 of 10 dead, C is 10 of 10 dead with a LOWER peak than either. The structural change is real (the old assembler drew the object as `type: rect, fill: var(--accent)` on the identical storyboard) but it moved the motion floor by nothing. What the floor measured was the RATE of localised change, and all three films key one reveal per three-second beat, so each holds still for two and a half seconds no matter who owns the DOM.
C measured worst, and the reason given was wrong: its slow 900x420 to 760x360 drift was called "spread and gentle, the definition of the ambient motion the gate refuses to count". It is not: a size-only key on a solid object changes a thin ring at its edge, which pairProfile's share metric reads as LOCAL, the opposite classification. Rebuilt to the same description by a since-retired A/B lab tool, the C variant measures BETTER than B, not worse, and a variant with real translation better still; the two runs cannot be reconciled, so the C number is fixture-dependent. What survives is the half that reproduces: A and B measure the same, so structure alone buys nothing. Two different problems were being treated as one: the PATH (can a thing survive a cut and be drawn as itself) and the DENSITY (how many overlapping keyed events per second). This work fixed the first; the second is the motion decider's, and no amount of the first substitutes for it.
holds: harness/author/assemble.mjs

## 609. a prop set and never read fails the render, and a comment naming the prop is not proof it is read
The new object layer carried `radius: chain[0].in.radius ?? 4`, a default inherited from the placeholder rect. As an `html` layer it refused to render at all: `core/registry/prop-audit.js` stops a build where a documented prop is accepted and then dropped (#428), which is the engine being right. The check that missed it was mine: `grep radius core/layers/html.js` returned a line, so it was called read, but the line was a COMMENT explaining that these box props are shared, and the builder consumes radius only through `chipBox`, which paints nothing without a surface prop beside it. Reading a match instead of the code around it is how a two-second check produces a confident wrong answer. The rect keeps its historic default so every existing film assembles byte-identically; a non-rect object now carries a radius only when the contract names one.
holds: harness/author/assemble.mjs, core/registry/prop-audit.js
...
Found by looking, not measuring: the highest scoring variant of the magnitude sweep reached a median of 1.61 and looked like a rendering fault, the entire scene, background and text together, drifting inside the canvas and exposing the edge. The number said it was the best film of the sweep. This is why every phase ends with the mp4 looked at, not only the table, and why the instrument gets fixed BEFORE it is promoted to a blocking gate: made binding with this hole, it would teach every author to slide the frame.
holds: quality/gates/motion-floor.mjs

## 612. going back one stage to fix a fragment silently deleted every directed camera move
`make assemble` is stage 5 of eight and `make dev-tool X=direct` is stage 6. The director writes a resolved `camera` track into the film; assemble's `PRESERVED_FILM_FIELDS` listed `cameraMove` and not `camera`, so re-running assemble to pick up an edited fragment dropped the whole camera track without a word. The stages are a documented, real order, but nothing said that stepping back through it destroys the stage in front of it. Found while checking byte-identity after an unrelated merge: the film lost a five-key camera array and the diff was the only thing that said so. assemble generates no camera of its own (its header says it is kept thin, on purpose, with no camera), so it never had a claim on the field. The same file already reports every preserved LAYER by name; film-level fields now go through the same report, so a preserved field is visible rather than assumed. Re-assembling a film is now idempotent, which it was not.
holds: harness/author/assemble.mjs, harness/author/motion-director.mjs
...
The self-test fixture had to change with it: it used a 60x34 block, 41% of the frame, as the proxy for "camera". Once a bounded object had to keep counting, a 41% block was on the wrong side of the line, because a panel that size is a design element, not a camera move. The fixture is now the full-bleed case that actually failed in a real film. The proxy was wrong, not the threshold.
holds: quality/gates/motion-floor.mjs
...
So the two pieces built separately are one mechanism: `move:` gives a beat motion that never lands, and `fragment: @ <placement>` gives it something to move that is not the entire frame. A film where every layer is full-bleed has no motion available except moving the frame, which is why "every beat is a whole separate page" and "the film reads as a slideshow" were never two problems. The first frame of the full-bleed `move:` test is empty: the pan slid the content clean off the canvas.
holds: harness/author/assemble.mjs, quality/gates/motion-floor.mjs, engine-doctrine/CRAFT/STORYBOARD-TEMPLATE.md

## 615. a new top-level directory is invisible to every gate that lists its scan surface
Moving the lightfield research rig out of `harness/author/` into a new `tools/` directory was a pure `git mv`, and it silently took 1,622 lines out of the reach of three checks at once. Five files hardcode which directories they walk, and a new one is not in any of them until somebody says so: `quality/gates/doc-refs.mjs` matched `scripts|core|verify|blocks` in two regexes, so every `node tools/...` reference in a doc simply stopped being verified; `quality/gates/code-quality.mjs`, `harness/dev/complexity.mjs` and `harness/dev/no-emdash.mjs` each carry their own array. Nothing failed; three gates just went quiet about a tenth of the code they were meant to cover.
`quality/gates/dead-branch.mjs` is the counter-example and the fix worth copying: its scan surface is DISCOVERED rather than listed, and its own comment says why, that the old array was "a map of where the code lived the day the gate was written" and that `formats/`, `sims/`, `mcp/` and `site/` were invisible by construction until it changed. Four files still have the map. Before any further directory moves, they either learn the new name or learn to discover it.
...
holds: quality/gates/doc-refs.mjs, harness/dev/no-emdash.mjs, harness/dev/complexity.mjs, quality/gates/dead-branch.mjs

## 616. the file's header said what it WAS, and two of us moved it on that sentence
<!-- doc-refs-allow: quality/gates/lib-test.mjs · the file this entry is about; later split by domain into tests/, so the name is history now, not a live path -->
`quality/gates/lib-test.mjs` is 7,485 lines and its first line reads "fast pure-JS asserts for the motion primitives in core/motion/motion.js. No browser needed (the primitives are pure)." On that sentence it was called a core unit test filed under gates and moved into `core/motion/`. The move failed on the first run: the file also imports eight modules from `harness/author/` (`arsenal`, `track`, `profiles`, `recency`, `schema-at`, `patch-motion`, `migrate-junctions`), plus `blocks/` and `blueprints/`. Inside `core/` those resolve to `core/author/...`, which does not exist, and even rewritten the result would be core depending on scripts, the dependency direction this repo exists to keep one-way.
The header was true when written; the file then grew for months and nobody updated the sentence, the ordinary life of a header comment. What was avoidable is trusting it: a two-line check (`grep "from '\.\."` and look at the roots) was available to the architect who recommended the move and the agent briefed to make it, and none ran it before the file was in flight.
...
holds: quality/gates/lib-test.mjs, engine-doctrine/CODEMAPS/SCRIPTS.md

## 617. the biggest move of the restructure was made against a tree 30 hours out of date
The `harness/` move was the restructure's largest single change: six folders, 259 files, 204 more files rewired, ~937 references. The agent reported it complete, with `make check` passing on a real film. Its branch forked from `ca0ecd43` and contained NONE of the day's 60 commits: it had moved `scripts/` as it existed before that work, so merging it would have silently reverted the eight-stage machinery, the motion fixes, the meter corrections and three earlier moves.
The tell was in its own report, and it read as diligence, not alarm: it said `make stage` does not exist, `.claude/settings.json` does not exist, `stage.mjs` does not exist under gates, and three directory-list gates still lack a `harness` entry, each "verified directly". Every one is true of `ca0ecd43` and false of main. A stale base does not produce obviously wrong output; it produces confident, specific output about a repository that no longer exists.
The brief said to run `git merge main` first. Saying it is not enough, and this is the fourth time a stale worktree has cost real work in one session (#607). What is needed is a check the agent must PASS and quote, not an instruction it can believe it followed: name a commit that must be an ancestor, and require `git merge-base --is-ancestor <sha> HEAD` shown in the report before any file moves.
...
(`harness/lib/design-spec.mjs`, `make dev-tool X=design-spec`). `harness/lib/stagekit.mjs buildKit` folds every
...
engine-doctrine/RULES per-rule files, a doc's own `when`/`answers` frontmatter, `harness/lib/safeguards.mjs`, and
hand-typed prose in `critics.mjs` each said "follow this rule" in a different shape. `engine-doctrine/CRAFT/rules/
...
migrated wholesale from `engine-doctrine/RULES`, so it carried records that were not motion at all (`caption-safe-
...
holds: harness/lib/craft-rules.mjs, harness/lib/finding-codes.mjs, harness/lib/safeguards.mjs, engine-doctrine/CRAFT/rules/
...
<!-- doc-refs-allow: quality/gates/copy.mjs · quoted from a film pass; the copy gate runs inside author-check, not as its own file -->
<!-- doc-refs-allow: make visuals · #NNN records a gate that was later culled -->
<!-- doc-refs-allow: quality/gates/visual-vocabulary.mjs · the entry records this gate's own deletion -->
...
## 630. the judge sheet labelled every frame with a time it did not come from
`quality/gates/beats-of.mjs` `beatsOf()` sampled each beat at `start + min(1.6, (next - start) * 0.55)` and then captioned the tile `beat N @<start>s`. Label and picture could be a second and a half apart. On the flagship launch cut that put a black mid-transition frame under `beat 8 @8.0s`, where the film is a full-bleed blue title: the eye reads the sheet, reports an empty stage at 8.0s, and the author opens 8.0s and finds nothing wrong. The judge is the one gate that SEES, so a caption it gets wrong is worse than a measurement it misses: a wrong measurement is absent, a wrong caption arrives in the same shape as a true finding and nobody downstream can separate them.
The fix: the label names the frame's own time and carries the beat's start beside it, `beat 8 @9.0s (starts 8.0s)`. One owner, one string, and both consumers (`judge.mjs`, `compare.mjs`) get it.
holds: none
