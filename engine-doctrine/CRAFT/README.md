...
| [FROM-GSAP.md](FROM-GSAP.md) | you know GSAP and want the vawe JSON that does the same thing | 15 side-by-side pairs, a GSAP tween/timeline/stagger/ease/motionPath/wiggle/loop next to the vawe field or JSON that does the same job, and why the shape differs (renderFrame(n) has no wall clock and no callbacks) |
