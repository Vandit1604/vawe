...
| 5 | **Depth-map reveal transition** | 5 | a flat UI frame parts in depth as the camera pushes, near pixels moving before far ones | none published. Tutorials state only "keyframe the displacement intensity" | **MISSING** | Nearest is `displace` (`core/looks/presets.js:109`, turbulence freq 0.012, scale 14), which is noise-driven, and `uiParallax` (`core/surfaces/three-fx.js:295`), which needs the planes pre-split by hand |
| 6 | **2.5D multi-plane parallax** | 5 | flat layers at depths, near ones travel further | `depth: back` is 0.73x (its own blurb) | **SHIPPED** | `plane` modifier + a camera move; `uiParallax` (`core/surfaces/three-fx.js:295`) dollies a stack of captured planes. Recipes #12 agrees |
| 7 | **Text scramble / random letters** | 5 | characters cycle through junk and resolve into the word | resolve left to right; our scramble advances in 24 steps across the window (`core/kinetic/presets.js:380`) | **SHIPPED** | `decode` (`core/kinetic/presets.js:135`), deterministic so a seek is exact |
...
| 10 | **Logo reveal / motion identity** | 4 | the mark draws or assembles, one short system reused everywhere | none published | **SHIPPED** | `logoReveal` beat, `draw` type preset (`core/kinetic/presets.js:215`) |
...
| 12 | **Coloured / gradient text reveal** | 3 | colour sweeps through the letterforms as the line lands | `linear-gradient(100deg)` at `backgroundSize: 250%`, position swept 100% to 0 (`core/kinetic/presets.js:149`) | **SHIPPED** | `gradient` and `colorWave` (`core/kinetic/presets.js:149`, `:176`) |
...
whole frame evenly and never parts it. Our `displace` (`core/looks/presets.js:109`) is exactly that wrong shape,
...
(`core/surfaces/three-fx.js:295`) is the right neighbour to build beside: same texture plumbing, different pose.
