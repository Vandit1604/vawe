...
**ENGINE MAPPING** Partly present. `core/type/type.js` PRESETS (`node -e "import('./core/type/type.js').then(m=>console.log(m.PRESETS.length))"`) carries the kinetic presets, and `assemble` is
