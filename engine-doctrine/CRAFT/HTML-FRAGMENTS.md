...
- **The handles become an invention.** The storyboard's `motion:` line names the CSS selectors the
  engine will animate, and `engine-doctrine/CRAFT/STORYBOARD-TEMPLATE.md` says why the line exists in the field's
...
Before writing any by hand, **load the relevant [`engine-doctrine/CRAFT/`](README.md) guide** (how to choose
...
stylesheet: 34 themes get 34 different kits, never one kit in 34 colours. Every number in it traces back
...
a reference calls for is allowed (engine-doctrine/MISTAKES.md #621).
