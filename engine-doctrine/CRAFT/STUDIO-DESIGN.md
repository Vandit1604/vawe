...
| COPY | search by intent, not a browse grid | another engine `catalog --query` | 566 effects and 217 blocks cannot be browsed. Rank them against a plain-English description of the beat. `engine-doctrine/EFFECTS.md` and `blocks/index.mjs` are already the index. |
...
| REJECT | dragging a bar to retime a clip | another engine / NLE habit | A layer's window is set by `start`, by the beat it belongs to, and by junction binding in `core/timeline/junctions.js`. A drag would be a second way to say a start time, which is exactly the drift this repo keeps paying for. |
