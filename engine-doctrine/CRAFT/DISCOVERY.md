...
answers: "how to use the whole palette (the layer types `ls core/layers/` lists, plus the effect arsenal `make regen` owns the count of, blueprints, cuts): the always-visible primer, the search, the gap report, and the name-three-reject-the-first discipline; and why the search stays token-overlap, not embeddings"
group: crosscutting
---
...
- The vocabulary is far larger than any one author reaches for: the layer types `ls core/layers/` lists, the whole effect arsenal
