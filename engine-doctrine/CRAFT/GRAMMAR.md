...
- `[ref: make study-tool X=grammar DOC=1]`: a measured catalog, not a gate. Nothing here blocks. Regenerate from `grammar/*.json` via `make study`; never hand-edit this file.
...
> GENERATED by `make study-tool X=grammar DOC=1` from `grammar/*.json`. Do not edit this file: edit the JSON
...
| pin-prompt-expand | 6.04s | · | · | · | 0.21–0.21 | dark |
...
failure: `arc-space-swiping`, `arc-zero-chrome`, `example-madera`, `latch-recreation`, `make-it-move`, `ours-tc`, `pin-583145851797705243`, `pin-prompt-expand`, `pinref`, `together-chat` are built on travel and dissolves.
...
| # | in | len | ground | motion | what moves | what triggers the next |
|---|---|---|---|---|---|---|
| 1 | 0s | 4.54s | light 209.1 | 1.61 | camera does one continuous dolly-in/focus-rack the whole shot: blurred and tilted at 0s, sharp and closer by 4.54s, panning right as it goes; the sidebar list staggers in row by row; the prompt text types with a per-letter colour flash; the send button turns green on the click | the film opens already in motion (no prior joint) |
| 2 | 4.54s | 0.74s | light 221.3 | 1.18 | the sent window exits left/shrinks with heavy motion blur while the tagline's first two words slide in from the right, each its own colour | the send button click fires the exit |
| 3 | 5.28s | 0.72s | light 220.9 | 1.45 | static camera hold once the words land; only the background shapes drift slowly; the accent word 'taste.' desaturates to ink last | the tagline's own entrance completes |
| 4 | 6s | 0.53s | light 168.9 | 6.64 | the tagline exits left with heavy motion blur while the first swipe card slides in from the right, ground beginning to tint pale blue-grey | the tagline finishes its hold and is pushed out by the next act arriving |
| 5 | 6.53s | 1.59s | mid 125 | 7.1 | a slight continuous camera push on the card stack; each card exits left with motion blur as the next enters from the right, three times, the ground re-tinting with each swap | the hand cursor's presses on 'Like' cause each card's exit |
| 6 | 8.12s | 0.48s | light 220.2 | 1.33 | the last card's exit motion-blur overlaps the next tagline's first words sliding in from the right | the last card is dragged out of frame |
| 7 | 8.6s | 0.97s | light 219.5 | 1.68 | static camera hold once the words land; background shapes drift; 'love.' desaturates to ink last, then the whole line begins to blur | the second tagline's own entrance completes |
| 8 | 9.57s | 0.8s | light 174.1 | 5.32 | the tagline exits UP (axis change from the acts before it) while the results window rises on y from below, overlapping | the tagline finishes its hold and is pushed upward by the next act arriving |
| 9 | 10.37s | 0.71s | light 177.2 | 9.67 | a slow continuous camera dolly-in on the window as its heading types and the grid fills in tile by tile | the window reaches its risen position and the heading starts typing |
| 10 | 11.08s | 1.95s | light 221 | 0.3 | the window exits UP with motion blur; the marks fly in from scattered positions and land as letterforms one at a time; camera holds static once the wordmark lands | the results window's hold ends and it is pushed upward; the wordmark assembly is the film's closing beat, no further joint |
...
**Takeaway.** one continuous shot, one event near the 30-35% mark (the box growing to fit a typed prompt), everything else flat

**Carries across the cuts.** the prompt pill itself, held on screen the whole shot and only ever changing state

**The loud moment.** the pill's own expansion as the prompt types in and wraps to a second line

| # | in | len | ground | motion | what moves | what triggers the next |
|---|---|---|---|---|---|---|
| 1 | 0s | 6.04s | dark 53.6 | 0.21 | the pill only: fades in empty, types a prompt word by word, widens then grows a second line around 1.8-3.5s, then holds still to the end | nothing; one continuous shot, no next shot |
...
make study-tool X=grammar DOC=1                             # regenerates this page
```
