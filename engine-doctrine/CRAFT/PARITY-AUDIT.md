...
Uncommitted working note. Three of the seven effects in `engine-doctrine/CRAFT/SAAS-MOTION-DEMAND.md`, checked
...
so `presetOpts` on a decode layer is live rather than silently inert (`engine-doctrine/MISTAKES.md` #543; decode
...
**Eight shipped films DO change, argued frame by frame in `engine-doctrine/MISTAKES.md` #544**: all eight are
...
1. **The scale correction is applied, not printed.** `hold` is a key on the plane spec and writes the
   ...
   `engine-doctrine/MISTAKES.md` #544.
2. **`uiParallax` reads the texture's own aspect.** The planes are unit `PlaneGeometry` scaled in
   ...
   same two numbers. `engine-doctrine/MISTAKES.md` #545.
...
2. **The environment is in `studio()`, not in the scene that noticed it missing.** A metal body with
   ...
   **`fromScene`, not `fromEquirectangular`, and that is worth the sentence.** The first attempt built a
   ...
   `engine-doctrine/MISTAKES.md` #546.
...
call site instead of reporting it. `engine-doctrine/MISTAKES.md` #544.
...
`logoReveal` (retired with blueprints, engine-doctrine/MISTAKES.md) wrapped that: a bloom at attack 0.55 / decay 1.5 / peak 0.4
