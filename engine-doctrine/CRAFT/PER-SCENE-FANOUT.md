...
- Overkill for a one-scene film, a film with no continuous object, or anything you would author faster
  by hand in one sitting. Read engine-doctrine/CRAFT/SUBAGENT-BUDGET.md before spending the tokens a real fan-out
  costs.
...
`engine-doctrine/CRAFT/SUBAGENT-BUDGET.md`: fewer, larger agents beat one-per-item, and a fan-out that cannot
...
anthology, one holding a different device from `engine-doctrine/CRAFT/FILM-STRUCTURE.md`) can still use
...
briefing three agents, the four commands are pure overhead: `engine-doctrine/CRAFT/SUBAGENT-BUDGET.md`'s own
