...
| `SEARCH` | `eye-trace.mjs:267` (retired) | 1.5s | Rayner-style fixation/saccade duration research | a fixation runs roughly 200-400ms in scene viewing; 1.5s covers 4-8 fixations, more than one orienting glance | LOOSE. The eye finds SOMETHING inside one or two fixations (under 1s), so a 1.5s search window is generous rather than tight, but no source publishes "how long before a viewer gives up looking for the new focal point after a cut", so this stays an analogy. |
