...
- `engine-doctrine/MCP.md`: unreachable. `engine-doctrine/FRAMEWORK-AUDIT.md` was unreachable too;
  since deleted in the `docs/` to `engine-doctrine/` rename, along with `MAKEFILE-AUDIT.md` and
  `audits/`, so the finding no longer applies.
  <!-- doc-refs-allow: engine-doctrine/FRAMEWORK-AUDIT.md · deleted in the docs/ to engine-doctrine/ rename, named here only as history -->
...
2. The `CLAUDE.md` snapshot in circulation still carried an "Open audit findings" section pointing at
   `engine-doctrine/audits/GAUNTLET-2026-08.md` and `gauntlet-2026-08.json`. Neither file exists; the section was
   deleted in commit `7d0f502`. A stale copy of the index outlived the index.
   <!-- doc-refs-allow: engine-doctrine/audits/GAUNTLET-2026-08.md · named here only to record that it does not exist -->
...
`@engine-doctrine/INDEX.md` import would move the bytes, not remove them.
...
links with one-line descriptions, not the filename or the root-path rule. A `engine-doctrine/INDEX.md` is the
...
- No guidance anywhere on converting an existing `engine-doctrine/*.md` tree into skills. Nothing addresses
...
(Ahrefs, above); and the shape it recommends is what `engine-doctrine/INDEX.md` already is.
