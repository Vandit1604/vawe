...
- This doc lists every primitive and prop the engine composes videos from: the layer types `ls core/layers/` lists, every
...
## Kinetic type (`core/type/type.js`): every name in `PRESETS` × char/word/line splits
...
(`engine-doctrine/MISTAKES.md` #59 rejected per-layer 3D on exactly that failure, having used the `perspective()`
...
- On a **photo or any opaque box**, the glow appears inside the image, around its highlights. It does
  ...
  blur the alpha channel. See engine-doctrine/MISTAKES.md #115.)
...
- **Customization (defaults just work):** every look reskins to the theme with zero config. Quick:
  ...
  **`lookOpts: { strength, color, color2, colors, grain, vignette }`**. `strength` is a master dial.
  ...
  There is no `warmth`; it was advertised in three places and no pass ever read it (`engine-doctrine/MISTAKES.md` #351).
...
three.js dependency would earn itself; see `engine-doctrine/ROADMAP.md`.
