...
holds: gated (make audit, contrast; HARD fail per engine-doctrine/CRAFT/COLOR.md)
...
`make audit` measures contrast on the COMPOSITED pixels, at the exact frame the text holds, not on the
...
| a 150px headline under 3:1 | fails `make audit` (HARD) |
