...
*Companion machine-readable index: `engine-doctrine/animation-db.json` (same data). Brand onboarding: `make study-tool X=brandspec URL=…` (real fonts, weights, tokens) + `make study-tool X=palette IMG=…` (eyedropped dominance) → hand-author `themes/<name>.json`. The single `make brandkit` that once did both was removed with the templates; `dna/<name>.json` never shipped.*
