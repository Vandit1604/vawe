...
import { buildLlmsTxt, OUT } from '../../harness/author/llms-txt.mjs';
...
for (const name of LAYER_REGISTRY.names) {
  assert.match(text, new RegExp('`' + name + '`'), `films/llms.txt is missing the layer type "${name}"`);
}
...
assert.equal(committed, text, 'films/llms.txt is stale: run `node harness/author/llms-txt.mjs` and commit the result');
...
assert.ok(!text.includes('\u2014'), 'films/llms.txt contains an em-dash (U+2014)');
