...
{
  const theme = { look: { bgDefault: [{ preset: 'paper' }, { preset: 'dark' }, { preset: 'accent' }] } };
...
{
  const theme = { look: { bgDefault: [{ preset: 'paper' }, { preset: 'dark' }] } };
...
{
  const theme = { look: { bgDefault: { preset: 'paper' } } };
  const bg = expandThemeRotation([{ use: 'theme' }], theme, table([2, 4]), 6);
