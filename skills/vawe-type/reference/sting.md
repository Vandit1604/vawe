...
Always the CONTINUOUS-ACTION shape (`harness/author/type-spines.mjs`'s `continuousObject`): the mark,
named as the one object, on a hand-keyed track from `track.mjs`'s measured `pan`/`blast` shapes, `black`
bg, zero transitions. Draw your real `svg` mark, not a placeholder box; the track and the
