...
**Which rows the brief already filled.** If `make quiz-apply` ran, it wrote orientation, duration,
the arc and `threads:` into the storyboard's frontmatter, and `make quiz-look` settled the direction
...
(TASTE-RULES) so `make direct` can check the effect against the intent.
...
opposite:
- **Assets: capture, never recreate.** `make capture` a mark/mascot/illustration, or crop it from
