---
name: vawe-effects
description: "Shows the WHOLE effect arsenal to pick from by need, instead of defaulting to rise+fade. Load while authoring a scene JSON when an effect must be chosen: a kinetic reveal, a living background, a border-beam/shine, a shape-morph logo, a paint field (aurora/meteor/matrix), a cut/sting/seam. Points at the generated engine-doctrine/EFFECTS.md (regenerated by `make effects`, so its count of effects and families never drifts from the code)."
stage: design
---
